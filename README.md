...
### 改訂版①
####　追加したもの
https://www.notion.so/ms-engineer/3568f7a036288097bc38f5d6a99bdf27
...
### 改訂版②
#### 追加したもの
<img width="1961" height="1020" alt="マッピング成功3" src="https://github.com/user-attachments/assets/6c865ebf-990c-497c-ab39-595c52633582" />
<img width="1908" height="1006" alt="プロントエンド200OK" src="https://github.com/user-attachments/assets/604c9c43-e2f5-4da7-9bd7-75de24c2d42f" />

#### [フロントエンド] (GPS検知: 神奈川県横浜市)
####       │
####       ▼ (HTTP POST /api/v1/chat)
#### [バックエンド: chat.py] ──(1. 地名クレンジング)──> "横浜"
####       │
####       ├─(2. 外部天気API: OpenWeatherMap) ──────> "曇りがち, 26°C"
####       │
####       ├─(3. RAGサービス: ChromaDB / Tavily) ────> 横浜のこだわり/最新トレンドを検索
####       │
####       ▼ (4. プロンプト生成 & OpenAI gpt-4o-mini)
#### [ AIエージェント ] (現在地・気候・ユーザー要求のコンテキストをすべて理解)
####       │
####       ▼ (5. レスポンス返却: JSON {"response": "..."})
#### [フロントエンド] (元の可愛いUIの `msg.text` にマッピング・描画)

### 改訂版③

<img width="1919" height="970" alt="image" src="https://github.com/user-attachments/assets/4ab124a8-6c77-429e-abee-425026701929" />

#### 【 📱 フロントエンド（app/chat/page.tsx & ChatComponent.tsx）】

#### 1. 【データ確定】
####    - 画面上部のお天気カードが「27°C」を取得。

#### 2. 【データスルーパス（最重要）】
####    - チャット送信時、テキストと一緒に{ main: { temp: 27 }, ... } の天気JSONを固める。
####    - ⚠️「Turbopackによる設定無視バグ」を回避するため、FastAPI(8000番)へ絶対パスでダイレクトPOST。

#### 3. 【強制クレンジング（パース成功）】
####    - <ReactMarkdown> を導入。
####    - さらに、LLMが癖で出力してしまう `###` や `**` を `cleanMessageText()` で跡形もなく消し去り、無垢でスッキリしたテキストを描画。
#### 
####          │
####          ▼ (ダイレクトPOST通信：http://127.0.0.1:8000/api/v1/chat)
####          │
#### 
#### 【 ⚙️ バックエンド（main.py & rag_service.py）】
#### 
#### 4. 【開通＆CORS許可】
####    - main.py がルーター重複登録を防ぎつつ、/api/v1 prefixで開通。
####    - 3000番ポートからのCORS通信を許可。
#### 
#### 5. 【プロンプト完全固定（データ完全同期）】
####    - rag_service.py のプロンプト最上部に「【絶対遵守ルール】」を追加。
####    - ⚠️「嘘の24°Cを喋るな、画面の27°Cと寸分の狂いもない整合性を取れ！」とLLMに命令。
#### 
####          │
####          ▼ (LLMへ「27℃・ランニング・最新トレンド」の3つを統合入力)
####          │
#### 
#### 【 🤖 LLM (OpenAI API) 】
#### 
#### 6. 【ルール通り回答】
####    - 「本日の日中は27℃と、汗ばむほどの暖かい陽気ですね！」と回答。捏造データ（23℃等）が完全に消滅。
####    - Markdown記号の周りに変なスペースを入れず、クレンジングしやすい綺麗な文章を出力。
